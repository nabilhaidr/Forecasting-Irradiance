...
|---|---|---|---|
| **S0-1** | 🟡 **Implementation/activation complete; observation 1/2** | IFS+AIFS archiver, schema, separate issue/valid/retrieval timestamps, manual smoke/full/catch-up, and first qualifying run [29560032016](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29560032016) are verified. Runs 29569622079, 29577578662, and 29586248092 failed during test collection before archiving because COV dependencies were absent. The workflow now installs both requirements sets; no post-fix schedule had run at this snapshot. | Obtain a successful post-fix schedule that commits and read-back-validates the next successive six-hour issue. Observation remains **1/2**; failed/pre-cutover/manual runs do not count. |
...
| **S0-4** | 🟡 **Partial** | Core site facts and specification sources have been supplied; repository config now records those core facts plus the S0-3 DNI·cosZ semantics decision, while the broader site/sensor inventory remains incomplete. | Populate complete `site_configuration` + `sensor_metadata`, including calculated/recorded GCR and bifaciality, RSI mounting, sensor serial/class/calibration data, and owner/date for each remaining null. |
...
| **S0-6** | 🟡 **Partial** | Repository skeleton plus NWP, COV, and DNI·cosZ tests exist; final local target is **198 tests**, and the manual S0-3 full-history workflow is green. | Add general push/PR CI plus `tests/leakage/test_no_future_leakage.py`, then record a green general-CI run. |
...
**Gate decision:** M0 is **not passed (1/7 fully accepted)**. S0-3 is accepted; S0-1 remains 1/2 and S0-2 remains 🟡 on historian confirmation. **S0-3 decision: COMPLETE.** **S0-4 decision: GO now.** Continue S0-4 discovery/metadata consolidation in parallel. **NO-GO for Phase 1 and for any model.**
