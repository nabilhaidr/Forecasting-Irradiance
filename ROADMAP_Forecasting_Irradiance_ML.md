...
> **Snapshot:** 2026-07-17 09:03 UTC. Legend: ✅ complete · 🟡 in progress · ⏭️ ready to start · ⬜ not started. Green means the deliverable **and** its acceptance evidence exist; it does not mean “the code was written.”

| ID | Status | Verified progress | Remaining gate / next action |
|---|---|---|---|
| **S0-1** | 🟡 **Implementation/activation complete; observation 1/2** | IFS + AIFS archiver, schema, hourly workflow, and separate issue/valid/retrieval timestamps are implemented; **166 local tests pass**. Manual smoke/full/catch-up succeeded. The first qualifying post-cutover [`schedule` run 29560032016](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29560032016) succeeded at 06:26 UTC and committed/read-back-validated AIFS issue `2026-07-17T00:00:00Z`; its IFS job skipped already-committed cycles. Shared Drive now has **11 IFS + 12 AIFS** paired Parquet/manifests at `gdrive:nwp`. | Observation is **1/2**. Verify a further qualifying scheduled event committing the next successive six-hour issue cycle. No second post-cutover scheduled run existed through 09:03 UTC; older/manual runs do not count. |
...
| **S0-3** | ⏭️ **GO now — implementation not started** | GHI, DHI, and DNI·cosZ history is available, and S0-2 supplies the data-backed `1min` diagnostic grid. A fixed timezone offset does not affect the exact-zero residual decision on coincident values. | Run the full-history residual test, publish the distribution/plot, and set `sensor_metadata.is_derived_tag`. Keep any zenith-conditioned interpretation provisional until historian clock semantics are independently confirmed. |
...
| **S0-6** | 🟡 **Partial** | Repository skeleton plus NWP and COV tests exist; **166 tests pass locally**. | Add general CI plus `tests/leakage/test_no_future_leakage.py`, then record a green CI run. |
...
**Gate decision:** M0 is **not passed (0/7 fully accepted)**. S0-2 has measured outputs but remains 🟡 on historian configuration confirmation; continue S0-1 and the remaining discovery tasks. **S0-3 decision: GO now.** This authorises the Sprint 0 diagnostic only. **NO-GO for Phase 1 and for any model.**
...
| **S0-3** | Required GHI, DHI, and DNI·cosZ source tags plus the data-backed diagnostic grid are available. | Run the full-history residual test, publish the residual/zenith evidence with clock caveat, and write `sensor_metadata.is_derived_tag`. |
...
| **S0-6** | Repository skeleton and NWP/COV test suites are implemented; **166 tests passed** at this snapshot. | Add general CI and `tests/leakage/test_no_future_leakage.py`, then record a green CI run. |
