...
| 1.8 | 2026-07-19 | **S0-5 complete.** Attached the strict full-history audit (run 29683909065): 19 months, 377 instantaneous XLSX workbooks + the 145-ZIP COV cross-check, 138 outage candidates, and all six operator leads corroborated by the sensor data. The empirical regime distribution shows `HIGHLY_VARIABLE` as the modal regime and no clean calendar season, confirming the §S0-5 box: the unseen-season split must be defined against the observed regime distribution. S0-5 → ✅ and M0 count 2/7 → 3/7; the historian-timezone caveat is carried, not blocking. No phase scope, estimate, or gate criterion changed. | *pending* |
