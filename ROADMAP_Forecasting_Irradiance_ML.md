...
> **Snapshot:** 2026-07-17 20:47 UTC. Legend: ✅ complete · 🟡 in progress · ⏭️ ready to start · ⬜ not started. Green means the deliverable **and** its acceptance evidence exist; it does not mean “the code was written.”

| ID | Status | Verified progress | Remaining gate / next action |
|---|---|---|---|
| **S0-1** | ✅ **Complete — observation 2/2** | IFS+AIFS archiver, schema, separate issue/valid/retrieval timestamps, manual smoke/full/catch-up, and first qualifying run [29560032016](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29560032016) (AIFS issue 2026-07-17T00:00Z) are verified. After the dependency fix, scheduled run [29592915322](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29592915322) committed and read-back-validated the successive six-hour cycle — AIFS 2026-07-17T06:00Z plus IFS 00:00Z/06:00Z — satisfying the observation gate; [29600290220](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29600290220) added AIFS 12:00Z and [29607833813](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29607833813) verified no uncommitted cycles remain. The three earlier dependency-collection failures do not count and did not need to. | None. The scheduled cadence continues as routine operations; regressions surface as workflow failures. |
| **S0-2** | 🟡 **Measured deliverables complete; historian confirmation open** | Strict CLI and full notebook runs reconciled **145/145 ZIPs** / **170 CSV entries** (163 populated, 7 empty), and produced identical hashes. The actual tag-in-header/value-in-column-B schema yielded **2,656,231** rows and **2,640,992** post-integrity events after **15,239** exact duplicates, with 12 order inversions and no malformed/conflicting rows. All 136 tags are characterised: **26 instantaneous**, **76 accumulation**, **34 meteorological**. Channel median active p50 evidence (slowest RSI 26.401 s) pins `canonical_freq`: **1min**; DNIcosZ EMI02 at 69.416 s is the one slower supported exception. Heartbeat is unresolved for all tags; configured max-report-time is `unknown`. Naive fractional timestamps are consistent with naive WITA from the substantial 06:00–18:00 activity window, but not configuration proof. Evidence: [`docs/phase0_cov_characterisation.md`](docs/phase0_cov_characterisation.md) and [`artifacts/phase0_cov/`](artifacts/phase0_cov/). | Obtain historian/SCADA configuration evidence for timestamp semantics and configured max-report-time/deadband before promoting to ✅. |
| **S0-3** | ✅ **Complete — independently measured** | Strict run [29589030480](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29589030480) analysed 145 ZIPs plus 156 instantaneous XLSX files (**8,027,772** historical raw valid rows) for EMI01–EMI04. All **40/40** direct/backward-only sensitivity cases are `measured`; historical direct active `n=319,332`, MAE 99.14 W/m², 1.15% within quantization. Plot/report/artifacts/tests exist and `sensor_metadata.is_derived_tag=false`. | None. Preserve the provisional zenith caveat until historian clock semantics are confirmed. |
...
| **S0-5** | 🟡 **Audit delivered; acceptance pending** | Delivered: [`docs/phase0_data_audit.md`](docs/phase0_data_audit.md) + [`artifacts/phase0_data_audit/`](artifacts/phase0_data_audit/). On the S0-2 `canonical_freq=1min` grid with backward-only ZOH, from 207 instantaneous XLSX workbooks (window **2025-12-01 → 2026-06-30**) plus the 145-ZIP COV cross-check: per-channel coverage/gap profiles, **63 outage candidates**, empirical monthly `k_c` and rule-based cloud-regime distributions **derived from the data** — the site is overwhelmingly cloudy (`CLEAR` ≤ ~13% of daylight), Dec–Feb carry the heaviest `OVERCAST`, Mar–Apr the most `HIGHLY_VARIABLE` and cloud enhancement. Operational corroboration: 1,212 maintenance rows (20 weather-station), 1,784 DCM outage/limitation intervals, 47 curtailment-days. Corrected S0-4 leads: **WS-3 GHI down March 2025 → 2025-06-25** (maintenance log 2025-03-06 "Communication Loss" → 2025-07-07 close), RSI WS3.1/3.2/3.3 removed 2025-06-30, WS4.1 2025-09-01, **WS4.2 2026-01-05 — directly corroborated in the sensor data from 2026-01-06**. New finding: a measured **WS-2 GHI outage March–May 2026**. | Attach the full-history CI run (`.github/workflows/s0-5-data-audit.yml`) so the Mar–Jun 2025 WS-3 outage and the 2025 RSI removals are sensor-measured rather than only log-corroborated, and confirm the historian timezone (S0-2). Acceptance stays 🟡 until both close. |
| **S0-6** | 🟡 **Partial** | Repository skeleton plus NWP, COV, DNI·cosZ, and site-metadata tests exist; final local target is **228 tests**, and the manual S0-3 full-history workflow is green. | Add general push/PR CI plus `tests/leakage/test_no_future_leakage.py`, then record a green general-CI run. |
...
**Gate decision:** M0 is **not passed (2/7 fully accepted)**. S0-1 and S0-3 are accepted; S0-2 remains 🟡 on historian confirmation. **S0-3 decision: COMPLETE.** **S0-4 decision: consolidation delivered; 🟡 pending serial/calibration certificates and mapping/geometry confirmations.** **S0-5 decision: audit delivered; 🟡 acceptance pending full-history CI coverage refresh and historian timezone confirmation** — the methodology, deterministic artifacts, empirical `k_c`/regime distributions, and operational corroboration are complete over the 2025-12 → 2026-06 window; the 2025 sensor coverage is attached by CI. See [`docs/phase0_data_audit.md`](docs/phase0_data_audit.md). **NO-GO for Phase 1 and for any model.**
...
| **S0-3** | Provenance-complete raw COV + historical XLSX test; no-future alignment; direct + four staleness cases; 40/40 stable `measured`; plot/report/artifacts/tests; `sensor_metadata.is_derived_tag=false`; run 29589030480. | — Complete. Retain the provisional zenith caveat pending historian clock confirmation. |
...
| **S0-5** | Coverage/gap/outage profiles, empirical monthly `k_c` and rule-based cloud-regime distributions read from the data, and maintenance/outage/curtailment corroboration are delivered in `docs/phase0_data_audit.md` + `artifacts/phase0_data_audit/` (window 2025-12 → 2026-06). | Attach the full-history CI run (2025 sensor coverage, incl. the Mar–Jun 2025 WS-3 outage) and confirm the historian timezone; acceptance stays 🟡 until both close. |
...
| 1.2 | 2026-07-17 | Added per-task acceptance checklists, refreshed the live S0-1 observation snapshot, and recorded the evidence-backed GO decision for S0-3. No phase scope, estimate, or gate criterion changed. | *pending* |
| 1.3 | 2026-07-17 | Recorded the strict full-history S0-3 measured decision, refreshed S0-1 scheduler evidence, and authorised S0-4 discovery. No phase scope, estimate, or gate criterion changed. | *pending* |
| 1.4 | 2026-07-18 | Recorded the S0-4 consolidation: populated canonical `site_configuration` + `sensor_metadata` with provenance, survey-derived geometry (pitch/GCR/module height), instrument inventories, RSI mounting survey, and owner/due-dated unresolved certificate evidence. S0-4 stays 🟡. No phase scope, estimate, or gate criterion changed. | *pending* |
| 1.5 | 2026-07-18 | Recorded S0-1 observation-gate completion: post-fix scheduled runs 29592915322/29600290220/29607833813 committed and read-back-validated the successive six-hour cycles; S0-1 → ✅ and M0 count 1/7 → 2/7. No phase scope, estimate, or gate criterion changed. | *pending* |
| 1.6 | 2026-07-18 | Recorded the evidence-backed GO decision for S0-5 (historical coverage audit) and refreshed the execution ledger to the post-S0-4 state. No phase scope, estimate, or gate criterion changed. | *pending* |
| 1.7 | 2026-07-19 | Delivered S0-5: `docs/phase0_data_audit.md` + `artifacts/phase0_data_audit/` with 1-minute backward-only coverage/gap/outage profiles, empirical monthly `k_c` and rule-based cloud-regime distributions derived from the site's own data (no textbook monsoon calendar), and maintenance/outage/curtailment corroboration. Corrected the S0-4 operator leads and recorded a new measured WS-2 GHI outage (Mar–May 2026). S0-5 acceptance 🟡 pending the full-history CI refresh and historian-timezone confirmation; M0 stays 2/7. No phase scope, estimate, or gate criterion changed. | *pending* |
