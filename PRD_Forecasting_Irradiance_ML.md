...
| Version | 2.2 (S0-7 escalation pack; evidence pending) |
...
| 1.2 | 2026-07-17 | Added per-task acceptance checklists, refreshed the live S0-1 observation snapshot, and recorded the evidence-backed GO decision for S0-3. No product requirement or gate criterion changed. |
| 1.3 | 2026-07-17 | Recorded the strict full-history S0-3 result, set DNI·cosZ metadata to independently measured, refreshed S0-1 scheduler evidence, and authorised S0-4 discovery. No product requirement or gate criterion changed. |
| 1.4 | 2026-07-18 | Recorded the S0-4 consolidation: populated canonical `site_configuration` + `sensor_metadata` with provenance, survey-derived geometry (pitch/GCR/module height), instrument inventories, RSI mounting survey, and owner/due-dated unresolved certificate evidence. S0-4 stays 🟡. No product requirement or gate criterion changed. |
| 1.5 | 2026-07-18 | Recorded S0-1 observation-gate completion: post-fix scheduled runs 29592915322/29600290220/29607833813 committed and read-back-validated the successive six-hour cycles; S0-1 → ✅ and M0 count 1/7 → 2/7. No product requirement or gate criterion changed. |
...
| 2.0 | 2026-07-19 | **S0-6 complete.** Added read-only general CI for every push and pull request plus the test-only `tests/leakage/test_no_future_leakage.py` harness. The harness proves canonical cutoff invariance, feature/target isolation, backward-only source use, staleness boundaries, verification lag, and the existing DNI·cosZ/minute-audit adapters without creating a feature builder, ensemble, baseline, or model. General CI run [29689204001](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29689204001) passed **272 tests** on Python 3.12 at implementation commit `316f439130c88bc08ad4093c869c3d69b73d584c`; M0 moves **3/7 → 4/7**. Historian timezone semantics remain an S0-2 caveat. No product requirement or gate criterion changed; Phase 1 and every model remain NO-GO. |
| 2.1 | 2026-07-19 | Re-audited S0-1–S0-6 against repository and live workflow evidence. Final-head General CI run [29689455820](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29689455820) is green at `7798d091f8a87fd05534a4d5e2edf1c7ecbdb46c`; scheduled NWP run [29689693626](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29689693626) is also green. Parent-task acceptance remains four green tasks plus two yellow tasks, while the unchanged Roadmap M0 wording has six of seven literal criteria evidenced. S0-7 is authorised to start now. No product requirement or gate criterion changed; Phase 1 and every model remain NO-GO pending formal M0 closeout. |
| 2.2 | 2026-07-19 | Prepared the sanitized S0-7 escalation brief (`docs/phase0_ot_security_escalation.md`) and structured pending record (`artifacts/phase0_ot_security/decision_record.json`), clarified the distinction between M0 escalation evidence and the final Phase-4 path decision, and removed unsupported `served=true` assumptions from the Master Context example. No external OT evidence, named decision owner, target date, or path/cadence/latency decision exists; the template is not acceptance evidence. S0-7 stays 0/1, parent acceptance stays 4/7, literal M0 evidence stays 6/7, and Phase 1/models remain NO-GO. |

### 1.2 Pre-Phase-1 Sign-off and M0 Commitments

| Role | Reason sign-off is needed |
|---|---|
| Plant Manager / Asset Owner | Scope, horizons, operational use of forecasts |
| OT / Network Security | Before Phase 1/M0 closeout: written OD-1 escalation evidence or a named OT decision owner plus target decision date. Final production-path/cadence/latency approval is required before any horizon is operationally served in Phase 4 (see §14, §33). |
...
> - **Evaluation candidate, not an approved SLA:** test whether **≥15 min** could be operationally servable and gate 5/10 min on a sub-minute push path (e.g. an approved read-only SFTP/OPC-UA push from the historian at ≤1-min cadence through the OT DMZ). Until OD-1 and measured latency prove otherwise, every `served` value remains provisional and defaults to `false`.
>
...
|---|---|---|---|---|
| **OD-1** | **What is the real-time data path?** Manual CSV, SFTP push, OPC-UA read-only, or something else? What cadence and latency does it *actually* achieve? | **YES — blocks operational nowcast serving / Phase 4; does not block offline development or backtesting after M0** | OT Security + Data Eng | Determines which horizons ship. See CON-4. |
...
|---|---|---|
| 1 | **OD-1 / CON-4 — the real-time data path** | Determines whether operational nowcasting is a *product* or a *backtest*. Offline development and backtesting can continue after M0, but no horizon may be served on an assumed path. **Escalate to OT security today.** |
...
| 7 | **Escalate OD-1 (the data path) to OT security.** | A written path/cadence/latency decision, or a named OT decision owner plus target decision date | The long pole. It will not resolve itself. Missing escalation evidence blocks M0; the final outcome gates operational serving in Phase 4, not offline development/backtesting. |

#### 42.4.1 OD-1 escalation evidence contract

The sanitized meeting pack is [`docs/phase0_ot_security_escalation.md`](docs/phase0_ot_security_escalation.md); its machine-readable state is [`artifacts/phase0_ot_security/decision_record.json`](artifacts/phase0_ot_security/decision_record.json). The current record is `pending`. The template is not acceptance evidence.

S0-7 is accepted only when the controlled external reference proves either (a) a written production-path decision with cadence and end-to-end latency, or (b) a named OT decision owner plus a written target decision date. Final path approval determines Phase-4 `served` flags; a scheduled decision can satisfy the M0 escalation criterion but cannot authorize operational serving.

### 42.5 Sprint 0 Execution Status — 2026-07-19

> **Operational snapshot:** 2026-07-19 15:01 UTC. Status legend: ✅ complete · 🟡 in progress · ⏭️ ready to start · ⬜ not started. A task receives ✅ only when its stated deliverable and acceptance evidence both exist.

| ID | Status | Progress and evidence | Remaining before ✅ |
|---|---|---|---|
| **S0-1** | ✅ **Complete — observation 2/2** | Archiver code, schema, hourly IFS+AIFS workflow, separate issue/valid/retrieval timestamps, manual smoke/full/catch-up, and the first qualifying post-cutover [`schedule` run 29560032016](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29560032016) (AIFS issue `2026-07-17T00:00:00Z`) are verified. After the dependency fix, scheduled run [29592915322](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29592915322) committed and read-back-validated the successive six-hour cycle (AIFS `2026-07-17T06:00:00Z` plus IFS `00:00Z`/`06:00Z`), [29600290220](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29600290220) added AIFS `12:00Z`, and [29607833813](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29607833813) verified no uncommitted cycles remain. The `committed` status is emitted only after the SHA-256 read-back and manifest verification path in `scripts/start_nwp_archiver.sh`. The three dependency-collection failures did not count as observations. | None. Scheduled cadence continues as routine operations. |
...
| **S0-3** | ✅ **Complete — independently measured** | Strict [full-history run 29589030480](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29589030480) verified 145 ZIPs plus 156 instantaneous XLSX files (**8,027,772** raw valid historical rows) across EMI01–EMI04. All **40/40** direct/backward-aligned sensitivity cases classified the channel as `measured`. Historical direct active evidence contains **319,332** samples with MAE 99.14 W/m²; only 1.15% is within the propagated quantization bound. The plot, deterministic artifacts, report, tests, and canonical `sensor_metadata.is_derived_tag=false` are present. | None. Zenith interpretation remains provisional until historian clock semantics are confirmed, but that caveat does not affect the fixed-offset-invariant derived-versus-measured decision. |
| **S0-4** | 🟡 **Consolidated; certificate evidence open** | The canonical config ([`configs/site_plts-ikn.yaml`](configs/site_plts-ikn.yaml)) now populates the complete `site_configuration` + `sensor_metadata` with per-field provenance: fixed racking, 10° tilt / 0° azimuth (survey measured 9–10°), POA co-planarity, survey row geometry (5.00 m collector width, 2.50 m front clear gap, both phases), derived `row_pitch_m=7.424` and `gcr=0.6735` (the centre-to-centre reading of the 2.50 m spacing is rejected as physically impossible), module height 1.46 m (mean of 12 surveyed points, 0.97–1.96 m), datasheet bifaciality 0.80±0.05 (Jinko JKM625N-78HL4-BDV), TMY-modelled `albedo_default=0.153` (low confidence, no albedometer), the WS-1..WS-4 inventory (Hukseflux SR20-D2 pyranometers, DR20 pyrheliometer on HQB-TG1 tracker — independent corroboration of the S0-3 measured decision) and WS-5 (SR30-M2-D1 Class A), plus the 12-point RSI mounting survey with operational removal notes. Schema validation (`src/characterisation/site_metadata.py`) and unit/integration tests are green. Evidence: [`docs/phase0_site_metadata_audit.md`](docs/phase0_site_metadata_audit.md) and [`artifacts/phase0_site_metadata/`](artifacts/phase0_site_metadata/). | Obtain per-unit serials and calibration date/due/factor certificates; confirm the RSI sensor model, DHI shading configuration, DR20 ISO class, EMI↔WS SCADA mapping, and design-drawing pitch/GCR/module-height values. Every open item is owner/due-dated in `unresolved_metadata`. |
| **S0-5** | ✅ **Complete — full history audited** | Strict full-history run [29683909065](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29683909065) audited **19 months (2024-12-21 → 2026-06-30)** across **377** instantaneous XLSX workbooks plus the **145-ZIP** COV cross-check on the S0-2 `canonical_freq=1min` grid with backward-only ZOH: per-channel coverage/gap profiles, **138 outage candidates**, empirical monthly `k_c`, and rule-based cloud-regime distributions **read from the data** — `HIGHLY_VARIABLE` is the modal regime (0.24–0.52 of daylight), `CLEAR` never exceeds 0.19, cloud enhancement (`k_c>1`) reaches 0.36, and **there is no clean calendar season** (February `OVERCAST` 0.291 in 2025 vs 0.337 in 2026; June `HIGHLY_VARIABLE` 0.377 vs 0.518), so **no monsoon calendar is assumed**. **All six operator leads are corroborated by the sensor data**: WS-3 GHI measured **2025-03-06 → 2025-06-23** (start identical to the maintenance "Communication Loss" ticket), WS-4 RSI_01 and RSI_02 dark one day after their reported removals. New findings: a **168-day WS-5 GHI outage** (2025-02-01 → 2025-07-18) and a **WS-2 GHI outage Feb–May 2026** with no operator record. Operational corroboration: 1,212 maintenance rows, 1,784 DCM intervals, 47 curtailment-days. Evidence: [`docs/phase0_data_audit.md`](docs/phase0_data_audit.md) + [`artifacts/phase0_data_audit/`](artifacts/phase0_data_audit/). | None. The historian-timezone caveat is carried inside the audit (every month/season label is provisional) per the S0-5 GO decision; closing it is S0-2 work, not S0-5. |
| **S0-6** | ✅ **Complete — general CI green** | Repository skeleton plus **272 passing tests** across unit, integration, leakage, notebook, and workflow contracts. The new test-only `tests/leakage/test_no_future_leakage.py` contains 12 deterministic tests covering cutoff invariance, deliberate future-fill detection, feature/target isolation, verification lag, backward-only source timestamps, missing/future/stale sources, inclusive DNI·cosZ staleness, and closed-minute-bin ZOH/coverage boundaries. The separate read-only `.github/workflows/ci.yml` runs on every push and pull request with pinned actions, `contents: read`, `persist-credentials: false`, no secrets or plant-data path, both pinned requirement sets, and the canonical suite. General CI run [29689204001](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29689204001) passed on Python 3.12 at `316f439130c88bc08ad4093c869c3d69b73d584c`. | None. The harness is deliberately test-only until Phase-1 feature/ensemble code exists. Closed-minute audit outputs are available at bin end, not their bin-start index; historian clock semantics remain provisional under S0-2. |
...
**Literal Gate M0 criterion coverage:** **6/7 evidenced** under the unchanged Roadmap §6 wording. M0-② is evidenced by the measured `canonical_freq=1min`; M0-④ is evidenced because `site_configuration` is populated and every recorded gap has an owner plus date. This 6/7 criterion count is deliberately separate from the stricter 4/7 parent-task acceptance count; neither count makes S0-2 or S0-4 green.
...
|---|---|---|
| **S0-1** | ✅ **Done (4/4)** | Archiver code/schema, IFS+AIFS schedule, separate issue/valid/retrieval timestamps, Shared-Drive SHA-256 read-back, and observation 2/2 remain accepted. Latest scheduled run [29689693626](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29689693626) is green; only routine monitoring remains. |
| **S0-2** | 🟡 **4/5** | Measured deliverables are complete: 145/145 ZIPs, 2,640,992 post-integrity events, per-tag timing/deadband evidence, deterministic report/artifacts, and `canonical_freq=1min`. Still needed: source-system proof of timestamp semantics, configured max-report-time, and configured deadband. |
...
| **S0-3** | Full raw-COV and longer-history XLSX provenance; backward-only COV alignment; direct + 60/120/300/900 s sensitivity; 40/40 stable `measured` cases; residual/zenith plot; deterministic artifacts; `docs/phase0_dni_cosz_test.md`; `sensor_metadata.is_derived_tag=false`; strict full-history run 29589030480. | — Complete. Keep the recorded provisional zenith caveat until historian clock semantics are confirmed. |
...
| **S0-6** | Repository skeleton, `tests/leakage/test_no_future_leakage.py`, read-only push/PR CI, and **272 passing tests** are implemented; green Python-3.12 run [29689204001](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29689204001) proves the workflow. The test-only harness covers cutoff invariance, feature/target isolation, verification lag, backward-only source use, missing/future/stale sources, DNI·cosZ alignment, and bin-end ZOH semantics. | — Complete. Keep the harness test-only until feature/ensemble code exists; the synthetic ordering checks do not resolve historian timezone semantics. |
