...
| Version | 1.7 (S0-5 historical coverage audit delivered) |
...
> **Operational snapshot:** 2026-07-17 20:47 UTC. Status legend: ✅ complete · 🟡 in progress · ⏭️ ready to start · ⬜ not started. A task receives ✅ only when its stated deliverable and acceptance evidence both exist.
...
| **S0-5** | 🟡 **Audit delivered; acceptance pending** | Historical coverage audit delivered — [`docs/phase0_data_audit.md`](docs/phase0_data_audit.md) + deterministic artifacts under [`artifacts/phase0_data_audit/`](artifacts/phase0_data_audit/). On the S0-2 measured `canonical_freq=1min` grid with backward-only ZOH, the audit derived per-channel coverage/gap/outage profiles, empirical monthly `k_c` and rule-based cloud-regime distributions **read from the data** (site is overwhelmingly cloudy — `CLEAR` never exceeds ~13% of daylight; Dec–Feb heaviest `OVERCAST`, Mar–Apr most `HIGHLY_VARIABLE`; **no monsoon calendar assumed**), and full-period maintenance/outage/curtailment corroboration (1,212 maintenance rows, 1,784 DCM intervals, 47 curtailment-days). Corrected the S0-4 leads: WS-3 GHI down **March 2025 → 2025-06-25**; RSI WS3.1/3.2/3.3 removed 2025-06-30, WS4.1 2025-09-01, WS4.2 2026-01-05 (the WS4.2 removal is directly corroborated in the sensor data on 2026-01-06). New finding: a measured WS-2 GHI outage March–May 2026. | Attach the **full-history CI run** (`.github/workflows/s0-5-data-audit.yml`) that measures the Mar–Jun 2025 WS-3 outage and the 2025 RSI removals from the sensors themselves; then confirm the historian timezone (S0-2). The local run measured only the 2025-12 → 2026-06 window. S0-5 acceptance stays **🟡** until both close. |
...
**Gate status:** M0 is **not passed (2/7 fully accepted)**. S0-1 and S0-3 are accepted; S0-2 remains 🟡 pending historian configuration evidence. **S0-3 decision: COMPLETE.** **S0-4 decision: consolidation delivered; 🟡 pending serial/calibration certificates and mapping/geometry confirmations.** **S0-5 decision: audit delivered; 🟡 acceptance pending full-history CI coverage refresh and historian timezone confirmation** — the methodology, artifacts, empirical `k_c`/regime, and operational corroboration are done over the 2025-12 → 2026-06 window; the full 2025 sensor coverage is attached by CI. See [`docs/phase0_data_audit.md`](docs/phase0_data_audit.md). **NO-GO for Phase 1 and for any model.**
