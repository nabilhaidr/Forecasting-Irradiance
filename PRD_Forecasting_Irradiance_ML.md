...
| Version | 1.2 (Sprint 0 task checklist and S0-3 readiness) |
...
| **S0-3** | ⏭️ **GO now — implementation not started** | GHI, DHI, and DNI·cosZ raw inputs are available across the characterised EMI tags, and S0-2 has pinned the diagnostic grid to `1min`. The exact-zero test on coincident channel values is invariant to a fixed timezone offset, so the open historian clock confirmation does not block starting this discovery task. | Produce the residual distribution and plot, set `sensor_metadata.is_derived_tag`, and label any residual-versus-zenith interpretation provisional until historian timestamp semantics are independently confirmed. |
