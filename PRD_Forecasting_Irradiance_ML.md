...
| Version | 1.4 (S0-4 site metadata consolidation) |
...
> **Operational snapshot:** 2026-07-17 15:33 UTC. Status legend: ✅ complete · 🟡 in progress · ⏭️ ready to start · ⬜ not started. A task receives ✅ only when its stated deliverable and acceptance evidence both exist.

| ID | Status | Progress and evidence | Remaining before ✅ |
|---|---|---|---|
| **S0-1** | 🟡 **In progress — implementation/activation complete; observation 1/2** | Archiver code, schema, hourly IFS+AIFS workflow, separate issue/valid/retrieval timestamps, manual smoke/full/catch-up, and the first qualifying post-cutover [`schedule` run 29560032016](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29560032016) are verified. That run committed and read-back-validated AIFS issue `2026-07-17T00:00:00Z`. Scheduled runs [29569622079](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29569622079), [29577578662](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29577578662), and [29586248092](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29586248092) failed before archiving because the workflow ran the full suite while installing only NWP dependencies. The workflow now installs both NWP and COV requirements; the fix is on canonical `main`, but no post-fix scheduled observation existed at this snapshot. | Observation remains **1/2**. Obtain a successful post-fix scheduled run that commits and read-back-validates the next successive six-hour issue cycle. The three dependency-collection failures do not count as observations. |
...
**Gate status:** M0 is **not passed (1/7 fully accepted)**. S0-3 is accepted; S0-1 remains at observation 1/2 and S0-2 remains 🟡 pending historian configuration evidence. **S0-3 decision: COMPLETE.** **S0-4 decision: consolidation delivered; 🟡 pending serial/calibration certificates and mapping/geometry confirmations.** **NO-GO for Phase 1 and for any model.**

#### Sprint 0 acceptance checklist

| Task | ✅ Verified progress | ⬜ Remaining before parent task is green |
|---|---|---|
| **S0-1** | Archiver code/schema, IFS+AIFS workflow activation, manual smoke/full/catch-up, Shared Drive read-back validation, and first qualifying scheduled issue cycle. | One further qualifying scheduled event must commit the next successive six-hour issue cycle; observation **1/2**. |
...
| **S0-5** | Historical raw files are available. | Audit coverage, gaps, outages/maintenance/curtailment, and empirical monthly `k_c`/regime distributions; publish `docs/phase0_data_audit.md`. |
