...
| Version | 1.3 (S0-3 measured decision and S0-4 readiness) |
...
| **S0-4** | 🟡 **Partially characterised** | The product owner has supplied coordinates, mean elevation, WITA timezone, fixed-tilt racking, 10° tilt, 0° azimuth, POA co-planarity, row dimensions, module-height ranges, no-albedometer status, sensor families, and source datasheets/workbooks. Repository config now records the core site facts plus the S0-3 DNI·cosZ semantics decision; the broader site/sensor inventory remains incomplete. | Consolidate the supplied facts into complete `site_configuration` and `sensor_metadata`; calculate/record GCR and bifaciality from the drawings/specifications; record RSI positions and per-sensor manufacturer/model/serial/class/calibration date, due date, and factor. Every remaining null needs a named owner and date. |
...
| **S0-6** | 🟡 **Partially complete** | Repository skeleton plus NWP, COV, and DNI·cosZ unit/integration/notebook/workflow tests exist; the final local suite target is **198 tests**. The S0-3 full-history workflow is green. | Add general CI for every push/PR and the required `tests/leakage/test_no_future_leakage.py`; obtain a green general-CI run. Task-specific workflows are not the full S0-6 deliverable. |
...
**Gate status:** M0 is **not passed (1/7 fully accepted)**. S0-3 is accepted; S0-1 remains at observation 1/2 and S0-2 remains 🟡 pending historian configuration evidence. **S0-3 decision: COMPLETE.** **S0-4 decision: GO now.** S0-4 is discovery/metadata consolidation and can proceed while S0-1/S0-2 evidence remains open. **NO-GO for Phase 1 and for any model.**
...
| **S0-4** | Core site, geometry, racking, sensor-family, and source-specification facts have been supplied. | Consolidate them into complete site/sensor metadata; calculate GCR and bifaciality; record RSI mounting, serial/class/calibration fields, and owner/date for each unresolved field. |
...
| **S0-6** | Repository skeleton and NWP/COV/DNI·cosZ test suites are implemented; **198 tests passed** at this snapshot. | Add general CI and `tests/leakage/test_no_future_leakage.py`, then record a green CI run. |
