...
|---|---|---|---|---|
| **S0-1** | **Start the NWP archiver** (ECMWF Open Data → Parquet, every cycle) | 🟡 **Implementation/activation complete; observation 1/2** | Code, schema, active IFS+AIFS workflow, separate UTC timestamps, manual smoke/full/catch-up, and first qualifying run [29560032016](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29560032016) are verified. Runs 29569622079, 29577578662, and 29586248092 failed before archiving because full-suite collection lacked COV dependencies; they do not count. The workflow now installs both NWP and COV requirements, but no post-fix scheduled observation existed at this snapshot. **Still required:** a successful post-fix schedule event committing and read-back-validating the next successive six-hour issue cycle. | Data Eng |
...
| **S0-4** | Site metadata audit → populate `site_configuration` + `sensor_metadata` | 🟡 **Partial** | Coordinates, elevation, timezone, fixed tilt, tilt/azimuth, POA co-planarity, row geometry, height ranges, no-albedometer status, sensor families, and source specifications were supplied. Repository config now records the core site facts plus the S0-3 DNI·cosZ semantics decision; the broader site/sensor inventory remains incomplete. Consolidate all supplied facts; calculate/record GCR and bifaciality; enter RSI mounting and per-sensor class/serial/calibration details; assign owner/date to every null. | Perf Eng / O&M |
...
| **S0-6** | Repo skeleton + CI + **the leakage harness** | 🟡 **Partial** | Skeleton plus NWP/COV/DNI·cosZ tests exist; final local target is **198 tests**, and the S0-3 full-history workflow is green. General push/PR CI and required `tests/leakage/test_no_future_leakage.py` remain absent. | ML Eng |
...
**M0 remains open: 1/7 tasks are fully accepted. S0-3 is complete; S0-1 is 1/2 and S0-2 remains 🟡 on historian confirmation.** **S0-3 decision: COMPLETE.** **S0-4 decision: GO now.** Continue metadata discovery without waiting for S0-1/S0-2 closure. **NO-GO for Phase 1 and all modelling.**
