...
| **Completed evidence** | PRD, Master Context, and Roadmap issued · source document audited · S0-1 implementation, tests, manual smoke/full/catch-up, Shared Drive cutover, and scheduler activation complete (the S0-1 task itself remains open pending its observation gate) · S0-2 measured deliverables, deterministic artifacts, report, and data-backed `canonical_freq=1min` complete (the S0-2 task itself remains open pending historian confirmation) |
| **In progress** | **S0-1** post-cutover scheduled-cycle observation · **S0-2** historian configuration confirmation · **S0-4** metadata consolidation · **S0-6** general CI/leakage harness completion |
| **Ready next** | **S0-3 DNI·cosZ diagnostic — GO now**, in parallel with the S0-1 observation and S0-2 historian follow-up |
...
> **Snapshot:** 2026-07-17 09:03 UTC. Legend: ✅ complete · 🟡 in progress · ⏭️ ready to start · ⬜ not started. Do not promote 🟡 to ✅ on implementation evidence alone when an operational acceptance gate remains.

| # | Task | Status | Evidence, gap, and next action | Owner |
|---|---|---|---|---|
| **S0-1** | **Start the NWP archiver** (ECMWF Open Data → Parquet, every cycle) | 🟡 **Implementation/activation complete; observation 1/2** | Code, schema, active workflow, **166 passing tests**, and successful smoke/full/catch-up are verified. The first qualifying post-cutover [`schedule` run 29560032016](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29560032016) succeeded at 06:26 UTC, committed and read-back-validated AIFS issue `2026-07-17T00:00:00Z`; its IFS job skipped only already-committed cycles. Shared Drive evidence is now **11 IFS + 12 AIFS** production issue cycles. The three UTC timestamps remain separate; scheduler is enabled at `gdrive:nwp`. **Still required:** one further qualifying schedule event committing the next successive six-hour issue cycle. No second post-cutover scheduled run existed through 09:03 UTC. | Data Eng |
...
| **S0-3** | The `dni_cosz` test: is `GHI − DHI − DNI·cosZ` ≈ 0 to machine precision? → `sensor_metadata.is_derived_tag` | ⏭️ **GO now — implementation not started** | Required GHI, DHI, and DNI·cosZ tags exist, and S0-2 supplies the `1min` diagnostic grid. The exact-zero residual decision can start because a fixed timezone offset does not change coincident values. Produce the residual distribution/plot and metadata boolean; treat any zenith-conditioned interpretation as provisional until the historian clock is confirmed. | Perf Eng |
...
**M0 remains open: 0/7 tasks are fully accepted. S0-2 is evidence-complete but remains 🟡 on historian confirmation.** **S0-3 decision: GO now.** This is a Sprint 0 diagnostic only. **NO-GO for Phase 1 and all modelling.**
