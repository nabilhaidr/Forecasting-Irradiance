...
| **Completed evidence** | PRD, Master Context, and Roadmap issued · source document audited · **S0-1 complete** (archiver live; observation 2/2 via post-fix scheduled runs) · S0-2 measured deliverables, deterministic artifacts, report, and data-backed `canonical_freq=1min` complete (the S0-2 task itself remains open pending historian confirmation) · **S0-3 complete** (`dni_cosz` independently measured, `is_derived_tag=false`) · **S0-4 consolidation delivered** (canonical `site_configuration` + `sensor_metadata` populated with provenance; certificate evidence open) · **S0-5 audit delivered** (`docs/phase0_data_audit.md` + `artifacts/phase0_data_audit/`: coverage/gap/outage, empirical monthly `k_c`/regime from the data, operational corroboration; acceptance 🟡 pending full-history CI + timezone) |
| **In progress** | **S0-2** historian configuration confirmation · **S0-4** certificate/mapping evidence collection (consolidation delivered) · **S0-5** full-history CI coverage refresh (2025 sensor coverage) · **S0-6** general CI/leakage harness completion |
| **Ready next** | **S0-5 full-history CI run** (`.github/workflows/s0-5-data-audit.yml`) to measure the Mar–Jun 2025 WS-3 outage and 2025 RSI removals from the sensors, in parallel with the S0-2 historian follow-up and S0-4 certificate collection |
...
| **S0-5** | Historical coverage audit → `docs/phase0_data_audit.md` | 🟡 **Audit delivered; acceptance pending** | Delivered on the `canonical_freq=1min` grid (backward-only ZOH): per-channel coverage/gap/outage profiles, empirical monthly `k_c` and rule-based cloud-regime distributions **read from the data** (site overwhelmingly cloudy — `CLEAR` ≤ ~13% of daylight; Dec–Feb heaviest `OVERCAST`, Mar–Apr most `HIGHLY_VARIABLE`; **no assumed Indonesian monsoon calendar**), and full-period maintenance/outage/curtailment corroboration. Corrected S0-4 leads (WS-3 GHI down Mar 2025 → 2025-06-25; RSI WS3.1/3.2/3.3 2025-06-30, WS4.1 2025-09-01, WS4.2 2026-01-05 — WS4.2 corroborated in-data on 2026-01-06); new finding: measured WS-2 GHI outage Mar–May 2026. Local window 2025-12 → 2026-06. Evidence: `docs/phase0_data_audit.md` + `artifacts/phase0_data_audit/`. **Remaining:** full-history CI run for the 2025 sensor coverage; historian timezone. | Data Eng |
...
**M0 remains open: 2/7 tasks are fully accepted. S0-1 and S0-3 are complete; S0-2 remains 🟡 on historian confirmation.** **S0-3 decision: COMPLETE.** **S0-5 decision: audit delivered; 🟡 acceptance pending full-history CI coverage refresh and historian timezone confirmation** — the methodology, deterministic artifacts, empirical `k_c`/regime distributions, and operational corroboration are complete over the 2025-12 → 2026-06 window; the 2025 sensor coverage is attached by CI. See [`docs/phase0_data_audit.md`](docs/phase0_data_audit.md). **S0-4 decision: consolidation delivered; 🟡 pending serial/calibration certificates and mapping/geometry confirmations.** **NO-GO for Phase 1 and all modelling.**
